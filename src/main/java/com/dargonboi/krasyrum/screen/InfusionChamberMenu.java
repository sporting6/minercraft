...
public class InfusionChamberMenu extends AbstractContainerMenu {
    ...
    public InfusionChamberMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.INFUSION_CHAMBER_MENU.get(), id);
        checkContainerSize(inv, 5);
        blockEntity = ((InfusionChamberBlockEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            this.addSlot(new SlotItemHandler(handler, 0, 20, 58)); //Imput
            this.addSlot(new SlotItemHandler(handler, 1, 46, 28)); //First Infusion
            this.addSlot(new SlotItemHandler(handler, 2, 97, 28)); //Second Infusion
            this.addSlot(new SlotItemHandler(handler, 3, 80, 87)); //Fuel
            this.addSlot(new ModResultSlot(handler, 4, 139, 58));             //Output
        });
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.INFUSION_CHAMBER.get());
    }
